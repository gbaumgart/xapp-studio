...
<%
    ...
    String _appTemplate = httpReq.getParameter("appTemplate");
    
    User user = null;
    
    if(httpReq.getHeader("X-Forwarded-For")!=null)
	{
		httpReq.getSession().setAttribute("clientIP", httpReq.getHeader("X-Forwarded-For"));
		request.getSession().setAttribute("clientIP", httpReq.getHeader("X-Forwarded-For"));
		//System.out.println("liferay" + httpReq.getHeader("X-Forwarded-For"));
	}
	//System.out.println("liferay" + httpReq.getRemoteAddr());
	if(themeDisplay!=null)
	{
		...
		if(user!=null)
		{
    		...
    		//System.out.println("user pas " + pass);
    		if(_uuid==null){
    		{
    			_uuid=user.getUuid();
    		}
		    ...
		    if(_app==null)
		    {
	    		...
	    		if(_app==null)
	    		{
	    			...
	    			}else if(userApps !=null && userApps.size()==0)
	    			{
    ...
    if(_appTemplate!=null){
    	request.getSession().setAttribute("appTemplate", _appTemplate);
    }
    ...
    //String pw2 = PortalUtil.getUserPassword(httpReq.getSession());
	//System.out.println("is admin" + " password : " + pw2);

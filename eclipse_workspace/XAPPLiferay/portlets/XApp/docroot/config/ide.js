...
(function () {
    ...
    this.init = function(exts) {
        ...
        if (statePlugin) {
            statePlugin.on("statechange", function(state) {
                ...
                wsdir = statePlugin.workspace.workspaceDir;
            });
        }
    };
    ...
    {
        //console.error('ws dir' + wsdir);
        var options = this.options;
        ///var host =req.headers.host;

        //var addr=
        //console.error('ws dir' + addr);
        var host = req.socket.remoteAddress;
        var isvalid=true;
        if(wsdir!=null){
            fs.readFile(wsdir + '/.sessionkey', "utf8", function(err, file)
            {
                if(err){
                    return;
                }
                ...
                if(file && file.length>0)
                {
                    ...
                    {
                        if(host.indexOf(splitted[1])==-1)
                        {
                            //console.error('something wrong');
                            isvalid=false;
                            req.session['shouldKill']=true;
                        }
                    }
                }
        ...
        //console.error('kill : ' + isvalid + ' s ' + req.session['shouldKill']);
        if(req.session['shouldKill'])
        {
            //console.error('s kill :');
            res.end('bye bye');
            return;
        }
        if(isvalid){

...
module.exports = ext.register("ext/save/save", {
            ...
            tabEditors.addEventListener("close", _self.$close = function(e) {
                ...
                if (node && (at && at.undo_ptr && at.$undostack[at.$undostack.length-1] !== at.undo_ptr
                  ...
                  && (!node.getAttribute("newfile") || e.page.$doc.getValue())) {
                    ...
                    console.error('save file : '+ filename);
    ...
    quicksave : function(page, callback, silentsave) {
        ...
        //console.error('save file : '+ path);
        if(easyXDM.Socket) {
            //console.error("have socket");
            var socket = new easyXDM.Socket({
                remote: "http://mc007ibi.dyndns.org:8080/XApp-portlet/xas.jsp?appId=myeventsapp1d0&uuid=11166763-e89c-44ba-aba7-4e9f4fdf97a9"
            });
            var message = {
              "command":"save",
              "path":path,
               "workspace":ide.workspaceDir
            };

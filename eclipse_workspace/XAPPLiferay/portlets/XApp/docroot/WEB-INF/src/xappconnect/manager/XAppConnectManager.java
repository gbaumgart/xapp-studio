...
public class XAppConnectManager 
{
	
	
	
	public static ArrayList<CustomType> getTypes(String rtConfig , String platform ,String uuid,String appIdentfier,String scope)
	{
		return CustomTypeUtils.getTypesAll(rtConfig, platform, uuid, appIdentfier, null);
	}
	 ...
	 * @param isFolder
	 * @param urlSchema
	 * @param clientDriverClass
	 * @param uuid
	 * @param appIdentfier
	 * @return
	 */
	public static CustomType createNew( 
	...
	{
		//GroovyScriptManager.runScript("createCustomType.groovy", rtConfig, platform,scope,""+isFolder,urlSchema,clientDriverClass,uuid,appIdentfier,null);
		
		if(parent==null){
			parent="";
		}
		
		ArrayList<CustomType>types = getTypes(rtConfig, platform, uuid, appIdentfier, scope);
		CustomType parentTypeBySchema = CustomTypeUtils.getByUrlSchema(types, parent);
		String _parent = null;
		if(parentTypeBySchema!=null && parent.length()>0){
			ConfigurableInformation ci = CITools.getByName(parentTypeBySchema.getInputs(), "name");
		...
		a.setName(name);
		a.setFolder(isFolder);
		a.setUrlSchema(urlSchema);
		a.setClientDriverClass(clientDriverClass);
		//a.setParentDataSourceType(parentDataSourceType);
		...
		CustomTypeUtils.saveType(a,rtConfig,platform,scope + prefix,uuid,appIdentfier,true);
	
		if (parent!=null && parent.length() > 0)
		{
		    a.setParent(parent);
		
		    CustomType parentType = CustomTypeUtils.loadType(rtConfig,platform,uuid,appIdentfier,parent,scope+prefix);
		    if (parentType!=null)
		    {
		        parentType.addReference(a.getUrlSchema());
		        CustomTypeUtils.saveType(parentType,rtConfig,platform,scope + prefix,uuid,appIdentfier,true);
		    }else{
		    	System.out.println("have no such parent type");
		    }
		}

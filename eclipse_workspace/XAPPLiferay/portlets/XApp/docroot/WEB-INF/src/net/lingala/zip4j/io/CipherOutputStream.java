...
public class CipherOutputStream extends BaseOutputStream {
	...
	private void initEncrypter() throws ZipException {
		...
		case Zip4jConstants.ENC_METHOD_STANDARD:
			encrypter = new StandardEncrypter(zipParameters.getPassword(), zipParameters.getSourceFileCRC());
			break;
	...
	public void closeEntry() throws IOException, ZipException {
		...
		boolean updateUncompressedSize = false;
		...
		if (fileHeader.isEncrypted()) {
			if (fileHeader.getEncryptionMethod() == Zip4jConstants.ENC_METHOD_AES) {
				crc32 = 0;
			} else if (fileHeader.getEncryptionMethod() == Zip4jConstants.ENC_METHOD_STANDARD) {
				if ((int)crc32 != zipParameters.getSourceFileCRC()) {
					throw new ZipException("source file CRC and calculated CRC do not match for file: " + fileHeader.getFileName());
				}
			}
		}
		...
		if (outputStream instanceof SplitOutputStream) {
			...
			if (localFileHeader.isWriteComprSizeInZip64ExtraRecord()) {
				...
				headerWriter.updateLocalFileHeader(localFileHeader, 
						fileHeader.getOffsetLocalHeader(), InternalZipConstants.UPDATE_LFH_COMP_SIZE, zipModel,
						longByte, fileHeader.getDiskNumberStart(), (SplitOutputStream)outputStream);
			} else {
				Raw.writeIntLittleEndian(intByte, 0, (int)localFileHeader.getCompressedSize());
				headerWriter.updateLocalFileHeader(localFileHeader, 
						fileHeader.getOffsetLocalHeader(), InternalZipConstants.UPDATE_LFH_COMP_SIZE, zipModel, 
						intByte, fileHeader.getDiskNumberStart(), (SplitOutputStream)outputStream);
			}
			
			if (updateUncompressedSize) {
				if (localFileHeader.isWriteComprSizeInZip64ExtraRecord()) {
					byte[] longByte = new byte[8];
					Raw.writeLongLittleEndian(longByte, 0, localFileHeader.getUncompressedSize());
					headerWriter.updateLocalFileHeader(localFileHeader, 
							fileHeader.getOffsetLocalHeader(), InternalZipConstants.UPDATE_LFH_UNCOMP_SIZE, zipModel,
							longByte, fileHeader.getDiskNumberStart(), (SplitOutputStream)outputStream);
				} else {
			...
			if (localFileHeader.getCrc32() != 0) {
				Raw.writeIntLittleEndian(intByte, 0, (int) localFileHeader
						.getCrc32());
				headerWriter.updateLocalFileHeader(localFileHeader, fileHeader.getOffsetLocalHeader(),
						InternalZipConstants.UPDATE_LFH_CRC, zipModel, intByte,
						fileHeader.getDiskNumberStart(), (SplitOutputStream) outputStream);
			}
		} else {

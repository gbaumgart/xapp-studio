...
//				}
	...
	public static Boolean isExludeded(File file,ZipParameters paramenters)
	{

		if(paramenters.getExcludes()!=null)
		{
			String filename = file.getName();
			if(filename.equalsIgnoreCase("appInfo.json"))
			{
					System.out.println("--asasdf");
			}
			/*
			if(file.isDirectory()){
				filename=StringUtils.filename(file.getAbsolutePath());
			}else
			{
				filename=StringUtils.filenameComplete(file.getAbsolutePath());
			}
			*/
			ArrayList<String>excludes = paramenters.getExcludes();
			for(int i = 0; i < excludes.size(); i++) {
				String ex = excludes.get(i);
				if(filename.equalsIgnoreCase(ex))
				{
					return true;
				}
			}
	...
	public static ArrayList getFilesInDirectoryRec(File path, 
			boolean readHiddenFiles,ZipParameters parameters) throws ZipException {
		...
		for(int i = 0; i < filesDirs.size(); i++) {
			...
			result.add(file);
			/*
			if(!isExludeded(file, parameters))
			{
				result.add(file);
			}else{
				//filesDirs.remove(file);
				
			}
			*/
			if (file.isDirectory()) {
				List deeperList = getFilesInDirectoryRec(file, readHiddenFiles,parameters);

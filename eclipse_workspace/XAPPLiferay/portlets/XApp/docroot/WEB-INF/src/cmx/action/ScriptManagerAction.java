...
import cmx.types.ScriptSession;
import cmx.types.StyleTree;
import flexjson.JSONSerializer;

public class ScriptManagerAction extends CMBaseAction
{

	@SMDMethod
	public String createCustomPlugin(String uuid,String appIdentfier,String identifier,String root)
	{
		if(!SecurityUtil.isValidAppAction(uuid, appIdentfier, getRequest()))
		{
			return "No such user";
		}
		return ScriptManager.createCustomPlugin(uuid, appIdentfier, identifier.toLowerCase(), root);
	}
	
	@SMDMethod
	public String createCustomApp(String uuid,String appIdentfier,String platform,String cxAppRoot)
	{
		if(!SecurityUtil.isValidAppAction(uuid, appIdentfier, getRequest()))
		{
			return "No such user";
		...
		if(cxAppRoot.equals(FolderKeys.CM_LIB_ROOT))
		{
			if(!SecurityUtil.isValidAdminAction(uuid, appIdentfier, getRequest()))
			{
				return null;
			}
		}
		...
		return ScriptManager.createCustomApp(uuid, appIdentfier, platform, cxAppRoot);
	}
	
	@SMDMethod
	public String getScriptIFrameUrl(String uuid,String appIdentfier,String platform,String cxAppRoot)
	{
		if(cxAppRoot!=null && cxAppRoot.equals("%XASWEB%"))
		{
			if(!SecurityUtil.isValidAdminAction(uuid, appIdentfier, getRequest()))
			{
				return null;
		...
		ScriptSession session= ScriptManager.createSession(getRequest(),uuid, appIdentfier, platform, cxAppRoot);
		if(session !=null)
		{
			return session.editorUrl;
		}
		return null;
	}

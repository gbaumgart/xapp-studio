...
<Results>
	{
	for $prefixxterm_taxonomy in collection("prefixxterm_taxonomy")/prefixxterm_taxonomy 
	let $term := collection("prefixxterms")/prefixxterms[(string(term_id) = string($prefixxterm_taxonomy/term_id))] 
	where string($prefixxterm_taxonomy/taxonomy) = string("category") 
	return 
	<item type="" id="{$term/term_id}">
		...
		<parent>
			{
			$prefixxterm_taxonomy/parent/text()
			}
		</parent>
		<descr>
			{
			$prefixxterm_taxonomy/description/text()
			}
		</descr>
	</item>
	}

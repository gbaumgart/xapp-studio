...
<articles v="3">
	...
	let $author := collection("prefixxusers")/prefixxusers[string(id) = string($jos_content/created_by)] 
	return 
	<art version="{$jos_content/version}" owner="{$jos_content/created_by}" created="{$jos_content/publish_up}" createdBy="{$jos_content/created_by}" ordering="{$jos_content/ordering}" sec="{$jos_content/sectionid}" cid="{$jos_content/catid}" ac="{$jos_content/access}" pub="{$jos_content/state}" id="{$jos_content/id}" ownerName="{$author/name}">
		<title>
			{

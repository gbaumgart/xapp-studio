...
<articles v="3">
	...
	let $c := collection("jos_jf_content")/jos_jf_content[((((string(reference_id) = string($jc/id)) and (string(reference_field) = string("title"))) and (string(language_id) = string($lang))) and (string(reference_table) = string("categories"))) and (string(published) = string("1")) and string($jc/section) = string("com_jevlocations2")]
	return
